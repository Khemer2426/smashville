...
export default class Template {
  ...
  _uiInit(options) {
    ...
    // Helper variables
    this._darkMode = "off";//options?.darkMode || "system";
  ...
  _uiHandleDarkMode(mode) {
    ...
    if (["on", "off", "system"].includes(mode)) {
      ...
      // if (rememberDarkMode) {
      //   darkMode = localStorage.getItem("oneuiDarkMode") || self._darkMode;
      // } else {
  ...
  _uiHandleTheme() {
    ...
    if (rememberTheme) {
      let themeName = localStorage.getItem("oneuiColorTheme") || false;
      ...
      localStorage.removeItem("oneuiColorTheme");
    }
    ...
    document.querySelectorAll('[data-toggle="theme"]').forEach((el) => {
      el.addEventListener("click", (e) => {
        ...
        if (rememberTheme) {
          localStorage.setItem("oneuiColorTheme", themeName);
        }
      });
    });
  }
